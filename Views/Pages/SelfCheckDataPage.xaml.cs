...
namespace 食品信息管理系统.Views.Pages
{
    ...
    /// 自检数据页面
    /// </summary>
    public partial class SelfCheckDataPage : Page
    {
        public event EventHandler? CloseRequested;

        private readonly SelfCheckDataService _service;
        private readonly DatabaseManager _db;
        private readonly int _currentRole;
        private readonly ObservableCollection<SelfCheckDataRecord> _records = new();
        private readonly ICollectionView _recordView;

        public SelfCheckDataPage()
            : this(0, new DatabaseManager("FoodEnterpriseIMS.db"))
        {
        }

        public SelfCheckDataPage(int currentRole, DatabaseManager db)
        {
            InitializeComponent();
            _currentRole = currentRole;
            _db = db;
            _service = new SelfCheckDataService();
        ...
        {
            if (item is not SelfCheckDataRecord record)
            {
                return false;
            }
            ...
            {
                var hit = Contains(record.SelfCheckId, keyword)
                          || Contains(record.InspectionId, keyword)
        ...
        {
            var dialog = new SelfCheckDataEditWindow(null, _records) { Owner = Window.GetWindow(this) };
            ...
            {
                MessageBox.Show($"新增自检数据失败：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }

        private void EditButton_Click(object sender, RoutedEventArgs e)
        {
            if (RecordGrid.SelectedItem is not SelfCheckDataRecord selected)
            {
                MessageBox.Show("请选择一条记录", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }

            var dialog = new SelfCheckDataEditWindow(selected, _records) { Owner = Window.GetWindow(this) };
            if (dialog.ShowDialog() != true)
            {
                return;
            }
            ...
            {
                MessageBox.Show($"更新自检数据失败：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void DeleteButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (MessageBox.Show($"确认删除自检数据 [{selected.SelfCheckId}] 吗？", "确认", MessageBoxButton.YesNo, MessageBoxImage.Question) != MessageBoxResult.Yes)
            {
                return;
            }
            ...
            {
                MessageBox.Show($"删除自检数据失败：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void ApplyButtonPermissions()
        {
            try
            {
                PagePermissionHelper.ApplyButtonPermissions(this, "self_check_data", _currentRole, _db);
            }
            catch
            {
                // ignore
